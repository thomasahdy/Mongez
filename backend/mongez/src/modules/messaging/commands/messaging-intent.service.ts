...
export class MessagingIntentService {
  ...
  private readonly rules: IntentRule[] = [
    ...
    },
    {
      type: 'INBOX',
      match: (t) => ({
        matched: /^(\/inbox|صندوق\s*الوارد|الوارد|inbox)$/i.test(t),
      }),
    },
    {
      type: 'DELEGATE',
      match: (t) => {
        // /delegate <userId> <endDate>  e.g. "/delegate sara 2026-07-01"
        const m = t.match(/^(?:\/delegate|تفويض)\s+(.+)$/i);
  ...
  async parse(rawText: string): Promise<ParsedIntent> {
    ...
    for (const rule of this.rules) {
      ...
      if (matched) {
        if (rule.type === 'CREATE_TASK' || rule.type === 'DELEGATE') {
