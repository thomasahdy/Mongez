...
const mapBackendNotification = (n) => {
  ...
  if (n.createdAt) {
    ...
    if (days > 0) {
      ...
      timeLabel = minutes === 1 ? "1 minute ago" : `${minutes} minutes ago`;
    }
  }

  const actions = [];
  if (n.status !== 'READ') {
    actions.push({ id: "read", icon: "fa-check", title: "Mark read" });
  }
  actions.push({ id: "archive", icon: "fa-box-archive", title: "Archive" });
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 dark:bg-slate-900">
        <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-red-700 max-w-md">
          <h3 className="font-bold text-lg mb-2">Failed to load Inbox</h3>
          <p className="text-sm">{error?.message || "Notifications are temporarily unavailable."}</p>
        </div>
      </div>
    );
  }
