...
export default function BillingPage({ setPath: propSetPath }) {
  ...
  if (loading) {
    return (
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900" aria-label="Billing settings">
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50" aria-label="Billing settings">
        <div className="mx-auto max-w-6xl px-6 py-6 space-y-6">
          ...
          {billingInfo && (
            <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
              <section className="rounded-[24px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm">
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Current plan</div>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(billingInfo.currentPlan?.features || []).map((feature) => (
                      <span
                        key={feature}
                        className="rounded-full bg-sky-50 dark:bg-sky-950/40 px-3 py-1 text-xs font-semibold text-sky-700 dark:text-sky-400"
                      >
                        {feature}
                      </span>
                    ))}
              ...
              <section className="rounded-[24px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm">
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Usage</div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {Object.entries(billingInfo.usageStats?.usage || {}).map(([key, value]) => (
                    <div key={key} className="rounded-2xl bg-slate-50 dark:bg-slate-800/40 p-4 border border-slate-100 dark:border-slate-800/60">
                      <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 dark:text-slate-500">{key}</div>
          ...
          {billingInfo && (
            <section className="rounded-[24px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm">
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Plan limits</div>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                {Object.entries(billingInfo.currentPlan?.limits?.quotas || {}).map(([key, value]) => (
                  <div key={key} className="rounded-2xl bg-slate-50 dark:bg-slate-800/40 p-4 border border-slate-100 dark:border-slate-800/60">
                    <div className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 dark:text-slate-500">{key} limit</div>
                    <div className="mt-2 text-lg font-bold text-slate-900 dark:text-slate-100">{String(value)}</div>
                  </div>
                ))}
          ...
          {billingInfo && (
            <section className="rounded-[24px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 p-6 shadow-sm">
              <div className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">AI usage</div>
