...
function Navbar() {
  ...
  return (
    <header className="landing-nav sticky top-0 z-50 border-b border-slate-200/90 bg-white/90 backdrop-blur-md" dir={isRTL ? "rtl" : "ltr"}>
      <div className={`mx-auto flex h-16 max-w-[1200px] items-center justify-between px-6 lg:px-8 ${isRTL ? "flex-row-reverse" : ""}`}>
        <BrandLogo
          to="/"
        ...
        <nav className={`hidden items-center gap-8 text-[14px] font-medium text-slate-500 lg:flex ${isRTL ? "flex-row-reverse" : ""}`}>
          {navLinks.map((link) => (
