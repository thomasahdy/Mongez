...
const NavItem = ({ href, icon, iconColor, label, badge, kbd, aiBadge, active, disabled, onClick }) => {
  ...
  const className = `flex items-center gap-2 px-2 py-[7px] rounded-lg text-[13px] font-medium transition-all duration-150
         ${active
  ...
  return (
    <NavLink
        to={href}
        onClick={onClick}
        className={({ isActive }) => `${className} ${isActive ? "bg-sky-100 text-sky-800 font-semibold dark:bg-sky-900/30 dark:text-sky-300" : ""}`}
      >
        {content}
      </NavLink>
  )
